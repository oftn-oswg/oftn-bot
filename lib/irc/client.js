...
var Client = exports.Client = function(profile) {
...
Client.prototype.get_user = function(name) {
	if (typeof this.users[name] === "undefined") {
		return this.users[name] = new User(this, name);
	}
	...
	data: function(chunk) {
		...
		while (this.buffer) {
			...
			if (message !== false) {
				...
				case "NICK":
					var oldname = User.parse_prefix(message.prefix).name;
				...
				case "JOIN":
					...
					var user = this.get_user(
						User.parse_prefix(message.prefix).name);
				...
				default:
					// If command is an error, print it.
					if (typeof message.command === "number") {
						if (message.command >= 400 && message.command < 600) {
							console.log("Error: (code "+message.command+"): " + message.message);
						}
					}
...
Channel.prototype.self_command = function(command, message, options) {
...
Channel.prototype.send_reply = function(user, message, options) {
	if (user === this) {
...
Channel.prototype.notice = function(message, options) {
	this.self_command ("NOTICE", message, options);
};


Channel.prototype.send_action = function(message, options) {
	var ctcp = "\x01";
...
Channel.prototype.join = function(password) {
...
Channel.prototype.set_topic = function(topic) {
	this.client.raw("TOPIC " + this.name + " :"+topic.replace(/\n/g, ' '));
};


var User = exports.User = function(client, name) {
	this.client = client;
	this.name = name;
};

Util.inherits(User, Channel);
...
User.parse_prefix = function(prefix) {
	var m = prefix.match(/^:(.*)!/);
	if (m !== null) {
		return {name: m[1]};
	}
