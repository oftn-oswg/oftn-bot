...
IRCBot.prototype.init = function() {
	...
	for (var i = 0, len = this.__profile.length; i < len; i++) {
		var client = new IRCClient(this.__profile[i]);
		...
		IRCBot.prototype.log.call(this, this.LOG_CONNECT, client.host, "Connecting...");
		client.connect();
	}
};
 ...
 **/
IRCBot.prototype.register_listener = function(regex, callback, options) {
 ...
 **/
IRCBot.prototype.register_command = function(command, callback, options) {
 ...
 **/
IRCBot.prototype.compile_command_listener = function() {
...
IRCBot.prototype.parse_intent = function(str) {
...
IRCBot.prototype.get_commands = function() {
	return Object.keys(this.__commands).sort();
};
...
IRCBot.prototype.set_maximum_truncate_message = function(message) {
	this.__maximum.message = message;
};
...
IRCBot.prototype.send_truncated = function(channel, reply, prefix, suffix) {
	...
	var truncated = false;
	var realmax = this.__maximum.length-this.__maximum.message.length;
	if (reply.length > realmax) {
		truncated = true;
		reply = reply.substr(0, realmax)+this.__maximum.message;
	}
	var splitted = reply.split(/[\r\n]+/g);
	if ((splitted.length > this.__maximum.lines) && !truncated)
		splitted[this.__maximum.lines-1] += this.__maximum.message;
	for (var i = 0; i < this.__maximum.lines && i < splitted.length; i++) {
...
IRCBot.prototype.listeners = {
	disconnect: function(client) {
		...
		IRCBot.prototype.log.call(this, this.LOG_CONNECT, client.host,
	...
	welcome: function(client) {
		...
		for (var i = 0, len = channels.length; i < len; i++) {
			IRCBot.prototype.log.call(this, this.LOG_JOIN, client.host, "Joining", channels[i]+"...");
			client.get_channel(channels[i]).join();
		}
	},
	join: function(channel) {
		IRCBot.prototype.log.call(this, this.LOG_JOIN, channel.client.host, "Joined", channel.name);
	},
	pm: function(user, text) {
	
		/* TODO: Add a `pm` boolean object to the context object passed to
		...
		if (user.name === "eboyjr") {
			...
			user.send(require("../sandbox/utils").pretty_print(result).substr(0, 300));
		} else {
	...
	message: function(channel, user, text) {
		...
		IRCBot.prototype.compile_command_listener.call(this);
		...
		if (command_matches) {
			...
			IRCBot.prototype.log.call(this, this.LOG_LISTENS, channel.client.host, channel.name+":",
					"<"+user.name+">", text);
			
			var hascommand = typeof this.__commands[command] !== "undefined";
			if (!hascommand ||
				(hascommand && this.__commands[command].options.allow_intentions)) {
				
				var intent_obj = IRCBot.prototype.parse_intent.call(this, full);
				if (intent_obj) {
		...
		var trimmed = Utilities.trim(text);
		...
		for (var i = 0, len = this.__listening.length; i < len; i++ ) {
			...
			if (this.__listening[i].options.allow_intentions) {
				var intent_obj = IRCBot.prototype.parse_intent.call(this, trimmed);
				if (intent_obj) {
