...
var SandboxUtils = require("../sandbox/utils");
...
IRCBot.prototype.listeners = {
	...
	pm: function(user, text) {
		...
		if (user.name === "eboyjr") {
			...
			user.send (SandboxUtils.pretty_print(result).substr(0, 200))
			return;
		}
		...
		for (var i = 0, len = this.__listening.length; i < len; i++ ) {
			...
			if (result) {
				...
				IRCBot.prototype.log.call(this, this.LOG_LISTENS, user.client.host,
		...
		if (command_matches) {
			...
			if (this.__commands[command] instanceof CommandListener) {
				if (this.__commands[command].options.allow_intentions) {
					var intent_obj = IRCBot.prototype.parse_intent.call(this, parameters);
					if (intent_obj) {
