...
IRCBot.prototype.init = function() {
	...
	for (var i = 0, len = this.profile.length; i < len; i++) {
		...
		client.on("welcome", Utilities.bind(this, this.listeners.welcome));
		client.on("join", Utilities.bind(this, this.listeners.join));
		client.on("message", Utilities.bind(this, this.listeners.message));
		client.on("pm", Utilities.bind(this, this.listeners.pm));
 ...
 * function with:
 *
 * callback(channel, user, text, 1st subpattern, 2nd subpattern, ...);
 **/
...
/**
 * Add a new command to listen for: callback is called with (channel, user, result)
		...
		if (matches && (index = matches[0].length)) {
			...
			for (var i = 0, len = this.commands.length; i < len; i++) {
				if (command === this.commands[i].command) {
					...
					this.commands[i].callback.call(this, channel, user, result);
			...
			if (command.length) {
				this.emit('command_not_found', channel, user, command);
				return;
			}
		}
		...
		for (var i = 0, len = this.listening.length; i < len; i++ ) {
			...
			if (result) {
				this.listening[i].callback.apply(this,
					[channel, user, text].concat(result.slice(1)));
