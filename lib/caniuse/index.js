...
CanIUseServer.prototype.readJSON = function() {
	File.readFile(this.filename, function (err, data) {
		if (err) {
			throw new Error("Error reading file: " + this.filename);
		}
		try {
			...
			if (this.attemptsLeft--) {
				Utils.puts("Will attempt to read the file again... (" + (5 - this.attemptsLeft) + "/5)");
				setTimeout(this.readJSON.bind(this), 1000);
			} else {
				Utils.puts("ERROR: Out of read attempts...");
			}
		}
	}.bind(this));
};
	...
	var req = HTTPS.request(this.dataSource, function(res) {
		...
		res.on('end', function() {
			try {
				...
				if (this.attemptsLeft--) {
					Utils.puts("Will attempt to fetch the file again in 10 seconds... (" + (5 - this.attemptsLeft) + "/5)");
					setTimeout(this.fetchJSON.bind(this), 10000);
				} else {
					Utils.puts("ERROR: Out of fetch attempts...");
				}
			}
		}.bind(this));
	}.bind(this));
