...
var FactoidHTTP = module.exports = function(db) {
	this.db = db;
	this.port = 3000;

	this.staticfiles = new Static.Server(__dirname+'/../../factoids/'/*, {cache: 24 * 3600}*/);
...
FactoidHTTP.prototype.query_database = function(obj) {
	...
	for (var i in db) {
		...
		if (typeof db[i].value !== "undefined") {
			...
			// Check if factoids are filtered and filter it
			if (obj.filter) {
				var terms = obj.filter.split(" "), found = false;
				var string = i + (db[i].value ? " "+db[i].value : "");
